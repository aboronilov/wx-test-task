...
```
Если вы указали **photo**, то данное фото перезальется на облачный севрис **Cloudinary** для последующей обработки на Frontend
Дополнительно реализован счетчик просмотров по полю **views** - оно будет увличиваться на 1 при каждом GET запросе к посту
Валидация отработает, если не указаны поля **title** или **content**
